...
func (dbc *DeleteBrokerCmd) Run() error {
	toDeleteBrokers, err := dbc.Client.ListBrokers(dbc.names)
	if err != nil {
		return err
	}
	...
	if len(toDeleteBrokers.Brokers) < len(dbc.names) {
		output.PrintError(dbc.Output, fmt.Errorf("%d names were not found", len(dbc.names)-len(toDeleteBrokers.Brokers)))
	}
